...
IMPORTANT: positions are stored in STEP coordinates (the controller's absolute
step counter), NOT micrometres.  That makes "go to position" deterministic --
it commands the exact step targets regardless of the current calibration, so
re-measuring ``um_per_step`` never moves a saved physical point.
"""
...
class Position:
    ...
    x/y/z are STEP coordinates (absolute controller step counts).
    """

    name: str = ""
    x: float = 0.0  # step coordinate
    y: float = 0.0
    z: float = 0.0
    used: bool = False
...
class PositionList:
    ...
    def store(self, index: int, x: float, y: float, z: float, name: str = "") -> Position:
        """Put step coordinates into slot ``index`` and mark it used."""
        self._check(index)
