...
The palette lives in aaltoview (`aaltoview.apps.theme`) and is
RE-EXPORTED here, not copied. The suite embeds the data viewer, and the theme
works by mutating ONE module-level COLORS dict in place (docs/DEVELOPER_NOTES.md section
6). Two copies would be two dicts: set_theme("light") here would leave the
viewer's widgets painting with dark colours.
"""
from pathlib import Path

from aaltoview.apps.theme import (C, COLORS, DARK, DEFAULT_THEME, LIGHT,  # noqa: F401
                                   STYLESHEET, THEMES, apply, apply_palette,
                                   build_stylesheet, set_theme)
