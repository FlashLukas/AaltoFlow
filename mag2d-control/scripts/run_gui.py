...
Connect to a running service (same PC or across the lab network):
    ...
    uv run scripts/run_gui.py --connect 192.168.1.42 --cmd-port 5575 --pub-port 5576
...
from mag2d.apps.gui import main as run_local, run_app
from mag2d.net.protocol import DEFAULT_CMD_PORT, DEFAULT_PUB_PORT


def main() -> int:
    ap = argparse.ArgumentParser(description="mag2d vector magnet GUI")
    ...
    client = Mag2dClient(host=args.connect, cmd_port=args.cmd_port, pub_port=args.pub_port)
    info = client.start()          # also pulls the service's config into client.cfg
    if not info:
        print(f"mag2d: no answer from {args.connect}:{args.cmd_port} -- is the service running?")
    if args.theme:
