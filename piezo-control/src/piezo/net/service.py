"""PiezoService -- owns the brain, serves commands, publishes status (§6).
...
class PiezoService:
    ...
    def start(self) -> None:
        ...
        self._stop.clear()
        self._threads = [
            threading.Thread(target=self._publisher, name="piezo-pub", daemon=True),
            threading.Thread(target=self._commander, name="piezo-cmd", daemon=True),
        ]
    ...
    def _dispatch(self, req: dict) -> dict:
        ...
        # -- motion ------------------------------------------------------ #
        if cmd == "move_axis":
            target = b.move_axis(P.parse_axis(req["axis"]), req["position"])
            return {"ok": True, "target": target}
        if cmd == "move_xy":
            targets = b.move_xy(req["x"], req["y"])
        ...
        # -- loop mode --------------------------------------------------- #
        if cmd == "set_closed_loop":
            state = b.set_closed_loop(P.parse_axis(req["axis"]), bool(req["enabled"]))
            return {"ok": True, "closed_loop": state}

        # -- velocity ---------------------------------------------------- #
        if cmd == "set_velocity":
            v = b.set_velocity(P.parse_axis(req["axis"]), req["value"])
            return {"ok": True, "value": v}
        if cmd == "set_ramp_mode":
            mode = b.set_ramp_mode(req["mode"])
