"""The client: talk to a Mag2dcalService, present a Controller-compatible facade.

The GUI (and your scripts) can hold a Mag2dcalClient exactly where they would hold
...
from .protocol import (DEFAULT_CMD_PORT, DEFAULT_PUB_PORT, TOPIC_STATUS, TOPIC_EVENT,
                       config_to_dict, apply_config_dict, calibration_to_dict,
...
class Mag2dcalClient:
    ...
    def set_stabilizer(self, enabled: bool):
        self._do({"cmd": "set_stabilizer", "enabled": bool(enabled)})

    def clear_fault(self):
        self._do({"cmd": "clear_fault"})

    # ---- calibration (same method names as the Controller, so the GUI is the
    # ---- same window whether the magnet is local or across the lab) --------

    @property
    def is_calibrated(self) -> bool:
        return bool(self.status().calibrated)

    def get_calibration(self):
        """The service's calibration as a real Calibration object (or None), so
        the viewer dialog does not care where it came from."""
    ...
    def set_calibration(self, cal) -> None:
        self._do({"cmd": "set_calibration", "calibration": calibration_to_dict(cal)})

    def calibrate(self, n_per_leg=None, dwell_s=None, v_max=None) -> None:
        """Start a calibration sweep. Returns at once; watch `state` for
        CALIBRATE and `calibration_progress` for how far it has got."""
        msg = {"cmd": "calibrate"}
        for key, value in (("n_per_leg", n_per_leg), ("dwell_s", dwell_s),
                           ("v_max", v_max)):
    ...
    def field_envelope_mT(self) -> float:
        """What the service will actually accept (info's live field_max_mT)."""
        return float(self.info().get("field_max_mT", self.cfg.limits.field_max_mT))
    ...
    def _wait_for(self, predicate, timeout_s: float, poll_s: float, what: str):
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(
                    f"mag2dcal: waited {timeout_s:g} s for {what}; state={st.state} "
