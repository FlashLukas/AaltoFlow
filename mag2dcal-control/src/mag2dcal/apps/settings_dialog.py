...
_TABS = [("control", "Control"), ("stabilizer", "Stabilizer"),
         ("calibration", "Calibration"), ("limits", "Limits"),
         ("interlock", "Interlock"), ("hall", "Hall probes"),
         ("temperature", "Temperature"), ("hardware", "Hardware"),
         ("sim", "Simulator"), ("ui", "Appearance")]

_HINTS = {
    "control": "Jump on the calibration, trim one way, then FREEZE inside tolerance/2. "
               "Gains are SIM-TUNED: retune them on the real magnet. Turning "
               "freeze_enabled off leaves an always-on PI (mag2d-control's behaviour), "
               "which on a magnet with hysteresis dithers.",
    "stabilizer": "The slow trim that runs while the field is held. It is the only thing "
                  "allowed to move a frozen output, at most once per period_s and only "
                  "outside deadband_mT. Keep it slow: it must not race the seek.",
    "calibration": "Where measured B(V) curves live and what a fresh sweep looks like. "
                   "dwell_s must be several time constants of the magnet, or every point "
                   "is recorded while the coil is still arriving.",
    "limits": "Setpoints outside the envelope are clamped with a warning. field_max_mT "
              "is |B|; with a calibration loaded the NARROWER of it and the measured "
              "range applies. The Hall probes read about +-186 mT.",
    ...
    "sim": "The simulated magnet. water_ok = False here trips the water interlock, "
           "which is a safe way to see a FAULT. hysteresis_mT and "
           "hysteresis_reversal_V set how badly a never-stopping controller "
           "limit-cycles; drift_mT_per_s gives the stabilizer something to do.",
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def _save_config(self):
        if not self._pull_into_cfg():
            return
        path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save config", "mag2dcal.ini", "Config (*.ini)")
        if path:
            self.cfg.save(path)
