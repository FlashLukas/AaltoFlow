...
Mag2dcalClient) and never cares which. A 50 ms timer reads status(); events cross
...
ring is the field envelope (which FOLLOWS the calibration), the dim arrow is
where you asked the field to be, the glowing amber arrow is where the Hall
probes say it is, and the small ring at the setpoint tip is the tolerance band
(green once the field has been inside it for stable_time). A glance tells you
magnitude, angle, and whether it has arrived; the dial stops pulsing when the
output freezes, which is the moment the field stops being pushed around.

Beyond mag2d-control's window this one adds the CALIBRATION card (run, view,
load, save, with a progress bar while a sweep runs) and the stabilizer switch.
"""
...
class VectorDial(QtWidgets.QWidget):
    ...
    def paintEvent(self, ev):
        ...
        if self._energized and self._stable:
            cap, col = ("STABLE - output frozen" if self._frozen else "STABLE"), _qcolor("ok")
        elif self._energized:
            cap, col = ("holding - output frozen" if self._frozen else "seeking"), \
                       _qcolor("accent_hi")
        else:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, ctrl, cfg: Config, remote: bool = False):
        ...
        self.cfg = cfg
        self._remote = remote
        self.setWindowTitle("mag2dcal - 2D vector magnet (calibrated)"
    ...
    def _build_sidebar(self) -> QtWidgets.QWidget:
        ...
        title = QtWidgets.QLabel("mag2dcal")
        ...
        self.stab_chk = QtWidgets.QCheckBox("Long-term stabilizer")
        self.stab_chk.setToolTip(
            "A slow deadbanded trim that corrects drift while the field is held.\n"
            "It is the only thing allowed to move a frozen output.")
        # .clicked, not .toggled: the status poll writes this box back every
        # 50 ms and must not fire the command again (gotcha #13).
        self.stab_chk.clicked.connect(lambda on: self._call(self.ctrl.set_stabilizer, on))
        olay.addWidget(self.stab_chk)
        col.addWidget(ocard)
    ...
    def _build_calibration_card(self) -> QtWidgets.QWidget:
        """The measured B(V) curve: what is loaded, and the four things you can
        do with it. This card is the whole difference from mag2d-control."""
        card, lay = _card("Calibration")
        self.cal_label = QtWidgets.QLabel("checking...")
        self.cal_label.setWordWrap(True)
        lay.addWidget(self.cal_label)

        self.cal_bar = QtWidgets.QProgressBar()
        self.cal_bar.setRange(0, 100)
        self.cal_bar.setTextVisible(True)
        self.cal_bar.setVisible(False)       # only while a sweep runs
        lay.addWidget(self.cal_bar)

        grid = QtWidgets.QGridLayout()
        grid.setSpacing(6)
        self.cal_run_btn = QtWidgets.QPushButton("Run calibration...")
        self.cal_run_btn.setObjectName("primary")
        self.cal_run_btn.clicked.connect(self._run_calibration)
        view = QtWidgets.QPushButton("View")
        view.clicked.connect(self._view_calibration)
    ...
    def _run_calibration(self):
        """Ask for the three numbers, warn plainly, then start the sweep.

        The sweep is NOT blocking: the service runs it in its control loop and
        publishes CALIBRATE plus a progress fraction, so this window stays live
        and the Zero button still aborts it.
        """
        cfg = self.cfg.calibration
        dlg = QtWidgets.QDialog(self)
        dlg.setWindowTitle("Run calibration")
        form = QtWidgets.QFormLayout(dlg)
        n = QtWidgets.QSpinBox(); n.setRange(2, 401); n.setValue(int(cfg.n_per_leg))
        dwell = self._dspin(0.0, 60.0, 2, 0.1, "s", float(cfg.dwell_s))
        vmax = self._dspin(0.1, abs(self.cfg.limits.ao_limit_V), 2, 0.5, "V",
                           float(cfg.v_max_V))
        form.addRow("Points per leg", n)
        form.addRow("Dwell per point", dwell)
        form.addRow("Sweep to", vmax)
        note = QtWidgets.QLabel(
            "Each axis is swept to +/- this voltage and back (the other axis at "
            "0 V), measuring the up and down hysteresis legs. THE MAGNET GOES TO "
            "FULL FIELD while this runs. Zero aborts it.\n\nThe dwell must be "
            "several time constants of the magnet, or every point is recorded "
            "while the coil is still arriving.")
        ...
        if dlg.exec() != QtWidgets.QDialog.Accepted:
            return
        est = 2 * (2 * n.value() + 3) * (dwell.value() + 0.5)
        self._on_event("info", f"calibration will take roughly {est:.0f} s")
        self._call(self.ctrl.calibrate, n.value(), dwell.value(), vmax.value())

    def _view_calibration(self):
        cal = self.ctrl.get_calibration()
        if not can_view(cal):
            self._on_event("warn", "no calibration to show; run or load one first")
            return
        CalibrationViewer(cal, self).exec()
    ...
    def _load_calibration(self):
        ...
        try:
            ...
            self._on_event("error", f"could not read {path}: {exc}")
            return
        # Goes over the wire when this is a remote client, so the SERVICE is the
        # one that ends up using it -- never a copy that only this window knows.
        self._call(self.ctrl.set_calibration, cal)

    def _save_calibration(self):
        cal = self.ctrl.get_calibration()
        if not can_view(cal):
            self._on_event("warn", "no calibration to save")
            return
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save calibration", "mag2dcal_calibration.json", "JSON (*.json)")
        if path:
            cal.save(path)
            self._on_event("info", f"calibration saved to {path}")
    ...
    def _apply_envelope(self):
        """Re-range the spin boxes from the LIVE envelope (the calibration can
        narrow it, so this is not simply cfg.limits.field_max_mT)."""
        lim = self.cfg.limits
        self._fmax = fmax = self._envelope()
    ...
    def _refresh_calibration_label(self):
        ...
        try:
            ...
            self._on_event("error", f"could not read the calibration: {exc}")
        if can_view(cal):
            self.cal_label.setText(cal.summary())
            self.cal_label.setStyleSheet(f"color:{COLORS['text']};")
        else:
            self.cal_label.setText(
                "no calibration -- the jump uses the straight line "
                f"B / {self.cfg.control.ff_mT_per_V:g} mT per volt")
            self.cal_label.setStyleSheet(f"color:{COLORS['accent_hi']};")

    def _envelope(self) -> float:
        """The largest |B| the magnet will accept, asked of the controller (or
        the client, which asks the service) rather than guessed from config."""
        try:
            return abs(self.ctrl.field_envelope_mT())
        except Exception:
            return abs(self.cfg.limits.field_max_mT)
    ...
    def _refresh(self):
        ...
        # ---- calibration card
        # The envelope and the curve only change when a calibration is loaded or
        # measured, and asking a REMOTE service for them costs a round trip -- so
        # refresh them on a CHANGE, not on every 50 ms poll.
        marker = (getattr(s, "calibrated", False), getattr(s, "describe_rev", None))
        if marker != self._cal_marker:
            self._cal_marker = marker
            self._apply_envelope()
            self._refresh_calibration_label()
        busy = s.state == "CALIBRATE"
        self.cal_bar.setVisible(busy)
        self.cal_run_btn.setEnabled(not busy)
        if busy:
            self.cal_bar.setValue(int(100 * getattr(s, "calibration_progress", 0.0)))
            self.cal_bar.setFormat("sweeping  %p%")
        self.stab_chk.blockSignals(True)
        self.stab_chk.setChecked(bool(getattr(s, "stabilizer", False)))
        self.stab_chk.blockSignals(False)

        self.dial.set_state(s.setpoint_bx_mT, s.setpoint_by_mT, s.measured_bx_mT,
                            s.measured_by_mT, self._fmax,
                            cfg.control.tolerance_mT, s.energized, s.field_stable,
                            frozen=getattr(s, "frozen", False))
