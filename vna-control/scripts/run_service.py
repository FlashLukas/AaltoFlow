"""Run the VNA service -- simulated (default) or the real Keysight PNA-X (--real).
...
default, clMag (5556) with --field clMag -- in BOTH modes: the real analyser
...
def build_analyzer(cfg: Config, real: bool):
    ...
    if real:
        ...
        return Analyzer(PnaVna(cfg), cfg)
    from vna.sim_system import build_sim_system
    return build_sim_system(cfg)[0]


def main() -> int:
    ap = argparse.ArgumentParser(description="VNA service: Keysight PNA-X (--real) or simulator")
    ap.add_argument("--host", default="0.0.0.0", help="bind address (default: all interfaces)")
    ap.add_argument("--cmd-port", type=int, default=DEFAULT_CMD_PORT)
    ap.add_argument("--pub-port", type=int, default=DEFAULT_PUB_PORT)
    ap.add_argument("--real", action="store_true",
                    help="drive the Keysight PNA-X over VISA instead of simulating")
    ap.add_argument("--visa", default=None, help="VISA resource/alias (default from config: N5222A)")
    ...
    where = {"mag2d": f"mag2d at {f.mag2d_host}:{f.mag2d_pub_port}",
             ...
             "clMag": f"clMag at {f.clMag_host}:{f.clMag_pub_port}"}.get(
        f.source, f"manual {f.manual_mT:g} mT at {f.manual_angle_deg:g} deg")
    if args.real:
        print(f"REAL VNA at VISA '{cfg.hardware.visa_resource}' -- field from {where}")
    else:
